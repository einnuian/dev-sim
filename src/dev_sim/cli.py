"""dev-sim CLI: Claude agent with tools for Git, GitHub repos, and pull requests.

Architecture (read top to bottom):
  main() loads .env, parses argv, then calls _run_agent().
  _run_agent() is a small tool-calling loop against the Anthropic Messages API: each assistant
  turn may request tool_use blocks; we execute them via _execute_tool(), JSON-serialize the
  result, and send tool_result blocks back until the model ends the turn.
  All filesystem and git side effects stay under the workspace root (see _resolve_under_workspace).
  GitHub REST calls use GITHUB_TOKEN from the environment (never passed to the model in tools).

Extension points: add a dict to _tool_specs(), handle the name in _execute_tool(), and optionally
add a private helper next to the other _github_* / git helpers.
"""
...
# First argument after "git" must be in this set so the agent cannot run arbitrary shell via git.
...
def _workspace_root(workdir: Path | None) -> Path:
...
def _run_agent(
    ...
    for _ in range(max_turns):
        ...
        for block in message.content:
            ...
            result = _execute_tool(str(tname), tinput, workspace, github_token)
            tool_results.append(
                {
                    "type": "tool_result",
                    "tool_use_id": tid,
...
def main() -> None:
    # Load .env from cwd and parents; does not override existing environment variables.
    load_dotenv()
    ...
    parser.add_argument(
        ...
        help=f"Model id (default: {DEFAULT_MODEL} or ANTHROPIC_MODEL)",
    )
    parser.add_argument(
        "--max-turns",
        type=int,
