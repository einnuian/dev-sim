...
To run **Claude coding agent → K2 review → optional follow-up** from team chat, start **`python -m dev_sim_bridge`** (port **8765**). Vite proxies **`/api/orchestrate`** (and **`/api/health`**) to that bridge. Use the chat **Agents** button for a short reminder.
